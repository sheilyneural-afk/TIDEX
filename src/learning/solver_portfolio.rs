...
use crate::foundation::low_rank_math::{solve_regularized_multi_case_low_rank, MAX_LOW_RANK};
...
const EXACT_PROBLEM_DOMAIN: &[u8] = b"TIDEX:EXACT-LS-PROBLEM:v1\0";
const EXACT_CANDIDATE_DOMAIN: &[u8] = b"TIDEX:EXACT-SOLVER-CANDIDATE:v1\0";
const PORTFOLIO_POLICY_DOMAIN: &[u8] = b"TIDEX:SOLVER-PORTFOLIO-POLICY:v1\0";
const PORTFOLIO_REPORT_DOMAIN: &[u8] = b"TIDEX:SOLVER-PORTFOLIO-REPORT:v1\0";
const CANDIDATE_EVALUATION_DOMAIN: &[u8] = b"TIDEX:CANDIDATE-EVALUATION:v1\0";
...
impl LeastSquaresProblem {
    pub fn new(inputs: Vec<Vec<f64>>, targets: Vec<Vec<f64>>) -> BrainResult<Self> {
        if inputs.is_empty() || targets.is_empty() || inputs.len() != targets.len() {
            return Err(BrainError::Invalid("solver_portfolio_problem_case_shape".into()));
        }
...
impl PortfolioPolicy {
    fn validate(&self) -> BrainResult<()> {
        ...
        {
            return Err(BrainError::Invalid("solver_portfolio_policy_invalid".into()));
        }
    ...
    pub fn digest(&self) -> BrainResult<SolverPortfolioPolicyDigest> {
        ...
        for value in [
            ...
            usize_value(self.limits.max_candidate_parameters, "max_candidate_parameters")?,
...
impl BackendDescriptor {
    ...
    fn validate(&self, external: bool) -> BrainResult<()> {
        ...
        {
            return Err(BrainError::Invalid("solver_backend_descriptor_invalid".into()));
        }
        Ok(())
    }
}
...
impl CandidateRepresentation {
    ...
    pub fn stored_parameter_count(&self) -> Option<usize> {
        match self {
            ...
            Self::Block { blocks, .. } => blocks
                .iter()
                .try_fold(0_usize, |total, block| total.checked_add(block.values.len())),
        }
    }
    ...
    pub fn exact_digest(&self) -> BrainResult<ExactSolverCandidateDigest> {
        ...
        if dense_elements > MAX_EXACT_CANDIDATE_DENSE_ELEMENTS {
            return Err(BrainError::Invalid("solver_candidate_digest_resource_limit".into()));
        }
...
impl SolverPortfolioReport {
    ...
    pub(crate) fn authenticate(&self) -> BrainResult<()> {
        ...
        if calculated != self.report_digest {
            return Err(BrainError::Integrity("solver_portfolio_report_digest_mismatch".into()));
        }
        Ok(())
    }
}
...
fn seal_portfolio_report(
    ...
    {
        return Err(BrainError::Integrity("solver_selected_evaluation_out_of_bounds".into()));
    }
...
impl ExternalCandidateProposal {
    ...
    fn with_message(
        ...
        {
            return Err(BrainError::Invalid("solver_external_proposal_message_invalid".into()));
        }
        Ok(Self {
            descriptor: BackendDescriptor::external(declared_implementation)?,
...
pub fn solve_with_portfolio(
        ...
        .find(|evaluation| evaluation.status == SolverStatus::BoundedUnknown)
    {
        ...
        (SolverStatus::Rejected, aggregate_rejection_reason(&evaluations)?)
    };
...
fn external_proposal_limit(
    ...
    if proposals.len() > policy.limits.max_external_proposals {
        return Some(EvaluationReason::ResourceLimit("solver_external_proposal_budget"));
    }
    ...
    match total_parameters {
        ...
        _ => Some(EvaluationReason::ResourceLimit("solver_max_total_external_parameters")),
    }
}
...
fn seal_with_unavailable_direct_solver(
        ...
        .find(|evaluation| evaluation.status == SolverStatus::BoundedUnknown)
    {
        ...
        return Err(BrainError::Integrity("solver_unavailable_path_without_unknown".into()));
    };
    seal_portfolio_report(
        problem,
        policy,
...
fn problem_limit(
    ...
    if problem.input_dimension() > limits.max_input_dimension {
        return Some(EvaluationReason::ResourceLimit("solver_max_input_dimension"));
    }
    if problem.output_dimension() > limits.max_output_dimension {
        return Some(EvaluationReason::ResourceLimit("solver_max_output_dimension"));
    }
    if problem
        .inputs
        .as_slice()
    ...
    {
        return Some(EvaluationReason::ResourceLimit("solver_safe_gram_magnitude"));
    }
    let working = problem
        .case_count()
...
fn direct_one_sided_jacobi_svd(
    ...
    for index in 0..component_count {
        ...
        if !singular_value.is_finite() {
            return Err(BrainError::Numerical("solver_direct_svd_singular_value_nonfinite".into()));
        }
...
fn verify_direct_svd(
    ...
    if !reconstruction_relative_error.is_finite() || !orthogonality_error.is_finite() {
        return Err(BrainError::Numerical("solver_direct_svd_verification_nonfinite".into()));
    }
...
fn run_direct_svd(
    ...
    if diagnostics.numerical_rank == 0 {
        ...
        return Ok(evaluate_candidate(backend, Some(0), candidate, problem, policy));
    }
...
fn measure_candidate(
    ...
    {
        return Err(BrainError::Numerical("solver_candidate_metrics_nonfinite".into()));
    }
    Ok(CandidateMetrics {
        absolute_residual,
...
pub fn measure_candidate_with_policy(
    ...
    if let Some(reason) = problem_limit(problem, policy) {
        return Err(BrainError::Invalid(format!("solver_measurement_resource_limit:{reason:?}")));
    }
    ...
    {
        return Err(BrainError::Invalid("solver_measurement_candidate_resource_limit".into()));
    }
    if single_candidate_evaluation_work_upper_bound(problem)
        .is_none_or(|work| work > u128::from(policy.limits.max_numerical_work_units))
    {
...
mod tests {
    ...
    fn exact_problem_digest_commits_to_shape_order_and_ieee_bits() {
        ...
        assert_ne!(positive_zero.digest().unwrap(), negative_zero.digest().unwrap());
    ...
    fn exact_candidate_and_policy_digests_commit_to_semantics() {
        ...
        assert_ne!(positive_zero.exact_digest().unwrap(), negative_zero.exact_digest().unwrap());
        assert_ne!(positive_zero.exact_digest().unwrap(), factored_zero.exact_digest().unwrap());
    ...
    fn ill_conditioned_full_rank_problem_selects_preplanned_spectral_backend() {
        ...
        assert_eq!(report.status, SolverStatus::Accepted);
        assert_eq!(report.cholesky_gate, Some(CholeskyGate::ConditionExceedsPolicy));
        assert_eq!(
    ...
    fn repeated_solve_is_deterministic() {
        ...
        assert_eq!(first.exact_digest().unwrap(), second.exact_digest().unwrap());
    ...
    fn zero_target_has_a_verified_zero_minimum_norm_solution() {
        ...
        assert_eq!(selected.metrics.as_ref().unwrap().relative_residual, Some(0.0));
        assert!(selected
            .candidate
            .as_ref()
            .unwrap()
    ...
    fn overflowing_tolerance_is_unknown_and_never_silently_accepts() {
        ...
        assert_eq!(report.reason(), &EvaluationReason::ResidualToleranceUnrepresentable);
    ...
    fn direct_svd_and_verifier_handle_large_finite_scaling() {
        ...
        let report = solve_with_portfolio(&problem, &exact_policy(), &[]).unwrap();
        assert_eq!(report.status, SolverStatus::Accepted);
        assert_eq!(report.cholesky_gate, Some(CholeskyGate::Float32ConversionUnsafe));
        assert_eq!(
            report.selected().unwrap().backend.role(),
            NumericalBackendRole::BuiltInDirectJacobiSvd
        );
    ...
    fn malformed_external_candidates_fail_closed() {
        ...
        for proposal in [wrong_shape, nonfinite] {
            ...
            let external = report
                ...
                .unwrap();
            assert_eq!(external.status, SolverStatus::Rejected);
            assert!(matches!(external.reason, EvaluationReason::InvalidCandidate(_)));
        }
    }
    ...
    fn external_relabeling_cannot_manipulate_selection() {
        ...
        assert_eq!(alpha_first.selected_evaluation, zulu_first.selected_evaluation);
        assert_eq!(
            alpha_first.selected().unwrap().candidate,
            zulu_first.selected().unwrap().candidate
        );
        assert_eq!(alpha_first.selected().unwrap().metrics, zulu_first.selected().unwrap().metrics);
        assert_eq!(alpha_first.selected().unwrap().backend.implementation(), "aaa.external");
        assert_eq!(zulu_first.selected().unwrap().backend.implementation(), "zzz.external");
    }
    ...
    fn external_candidate_order_cannot_change_selected_semantics() {
        ...
        assert_eq!(forward.exact_digest().unwrap(), reverse.exact_digest().unwrap());
    }
    ...
    fn gram_overflow_risk_is_a_bounded_unknown() {
        ...
        let report = solve_with_portfolio(&problem, &policy, &[]).unwrap();
        assert_eq!(report.status(), SolverStatus::BoundedUnknown);
        assert_eq!(report.reason(), &EvaluationReason::ResourceLimit("solver_safe_gram_magnitude"));
    }
    ...
    fn direct_svd_work_is_bounded_before_execution() {
        ...
        let report = solve_with_portfolio(&problem, &policy, &[]).unwrap();
        assert_eq!(report.status(), SolverStatus::BoundedUnknown);
        assert_eq!(report.reason(), &EvaluationReason::ResourceLimit("solver_max_svd_work_units"));
        assert_eq!(report.evaluations().len(), 1);
        assert_eq!(report.evaluations()[0].status(), SolverStatus::BoundedUnknown);
    }
    ...
    fn resource_exhaustion_is_unknown_not_numerical_rejection() {
        ...
        let report = solve_with_portfolio(&problem, &policy, &[]).unwrap();
        assert_eq!(report.status, SolverStatus::BoundedUnknown);
        assert_eq!(report.reason, EvaluationReason::ResourceLimit("solver_max_cases"));
        assert!(report.diagnostics.is_none());
    }
