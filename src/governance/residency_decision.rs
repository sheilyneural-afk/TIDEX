...
use crate::foundation::identity::{InquiryId, ResidencyDecisionRoundId, ResidencyTargetId};
use crate::foundation::security::verify_private_root;
use crate::knowledge::knowledge_engine::{
...
impl ResidencyDecisionPrecommit {
    ...
    fn calculate_digest(&self) -> BrainResult<ResidencyPrecommitDigest> {
        ...
        Ok(ResidencyPrecommitDigest::from_computed(Sha256Digest::digest_domain(
            PRECOMMIT_DOMAIN,
            &serde_json::to_vec(&projection)?,
        )))
    }
...
impl ResidencyDecisionRecord {
    ...
    fn calculate_digest(&self) -> BrainResult<ResidencyDecisionDigest> {
        ...
        Ok(ResidencyDecisionDigest::from_computed(Sha256Digest::digest_domain(
            DECISION_DOMAIN,
            &serde_json::to_vec(&projection)?,
        )))
    }
...
impl<'a> ResidencyDecisionAuthority<'a> {
    ...
    fn derive_decision(
        ...
        target_id: &ResidencyTargetId,
    ) -> BrainResult<(ResidencyDecision, Vec<ResidencyFactEvidence>, Option<ResidencySelectionBasis>)>
    {
        ...
        match self.knowledge_engine.plan(state)? {
            ...
            PlanningDecision::Terminal {
                ...
                let reasons = blockers
                    .into_iter()
                    .map(|(obligation_id, block)| ResidencyBlockReason::KnowledgeTerminalBlocked {
                        obligation_id,
                        block,
                    })
                    .collect();
...
impl<'a> ResidencyFactCollector<'a> {
    ...
    fn representability(
        ...
        let Some(value) = value else {
            return Ok(CandidateFactStatus::NotEstablished(CandidateEvidenceGap::Malformed));
        };
        self.evidence.push(ResidencyFactEvidence {
            kind: requirement.kind,
            claim_id: claim.id().clone(),
...
fn evaluate_fact_matrix(
    ...
    let ranked_dimensions = [
        (ResidencyDimension::Requirements, requirement_rank(facts.requirements)),
        (ResidencyDimension::Effects, effect_rank(facts.effects)),
        (ResidencyDimension::ExternalState, external_state_rank(facts.external_state)),
        (ResidencyDimension::Observability, observability_rank(facts.observability)),
    ];
...
mod tests {
    ...
    fn sealed_decision_detects_outcome_tampering() {
        ...
        let precommit_reference =
            reference("/authority/state/residency_decision/precommits/by-sha/precommit.json", 3);
        let mut record = ResidencyDecisionRecord {
            schema: ResidencySchema::Current,
    ...
    fn semantic_parsers_and_ranks_behave_exhaustively() {
        ...
        assert_eq!(parse_effects(PURE_EFFECTS_VALUE), Some(EffectSemantics::Pure));
        assert_eq!(parse_effects(BOUNDARY_EFFECTS_VALUE), Some(EffectSemantics::BoundaryEffects));
        assert_eq!(parse_effects(SOFTWARE_EFFECTS_VALUE), Some(EffectSemantics::SoftwareEffects));
        assert_eq!(parse_effects(UNKNOWN_SEMANTICS_VALUE), Some(EffectSemantics::Unknown));
        ...
        assert_eq!(requirement_rank(ExecutionRequirements::ClosedComputation), 0);
        ...
        assert_eq!(external_state_rank(ExternalStateSemantics::BoundaryManaged), 1);
        assert_eq!(external_state_rank(ExternalStateSemantics::SoftwareAuthoritative), 2);
        assert_eq!(external_state_rank(ExternalStateSemantics::Unknown), 2);

        assert_eq!(observability_rank(ObservabilitySemantics::WeightComplete), 0);
        assert_eq!(observability_rank(ObservabilitySemantics::BoundaryComplete), 1);
        assert_eq!(observability_rank(ObservabilitySemantics::SoftwareComplete), 2);
    ...
    fn getters_and_accessors_contract_verification() {
        ...
        let pred = KnowledgePredicate::BoolEquals {
            key: crate::knowledge::knowledge_engine::ObservationKey::parse("test_key").unwrap(),
            expected: true,
        };
