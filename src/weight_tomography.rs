...
use crate::linalg::{compensated_sum, stable_rms};
...
fn stable_mean(values: &[f64]) -> BrainResult<f64> {
    if values.is_empty() || values.iter().any(|value| !value.is_finite()) {
        return Err(BrainError::Invalid(
            "weight_tomography_mean_input_invalid".into(),
        ));
    }
...
fn weighted_nonnegative_mean(values: &[f64], weights: &[f64]) -> BrainResult<f64> {
    ...
    {
        return Err(BrainError::Invalid(
            "weight_tomography_weighted_mean_input_invalid".into(),
        ));
    }
    ...
    if !result.is_finite() || result < 0.0 {
        return Err(BrainError::Numerical(
            "weight_tomography_weighted_mean_nonfinite".into(),
        ));
    }
    Ok(result)
}
...
fn detrend_scale_normalized(values: &[f64]) -> BrainResult<(Vec<f64>, f64)> {
    if values.is_empty() || values.iter().any(|value| !value.is_finite()) {
        return Err(BrainError::Invalid(
            "weight_tomography_signal_invalid".into(),
        ));
    }
    let scale = values
        .iter()
    ...
    if !slope.is_finite() || detrended.iter().any(|value| !value.is_finite()) {
        return Err(BrainError::Numerical(
            "weight_tomography_detrend_nonfinite".into(),
        ));
    }
    Ok((detrended, slope))
}
...
fn neumaier_add(sum: &mut f64, correction: &mut f64, value: f64) -> BrainResult<()> {
    if !value.is_finite() {
        return Err(BrainError::Numerical(
            "weight_tomography_reduction_input_nonfinite".into(),
        ));
    }
    ...
    *sum = updated;
    if !sum.is_finite() || !correction.is_finite() {
        return Err(BrainError::Numerical(
            "weight_tomography_reduction_nonfinite".into(),
        ));
    }
    Ok(())
}
...
fn positive_dft(values: &[f64]) -> BrainResult<Vec<ComplexPair>> {
    if values.len() < 2 || values.iter().any(|value| !value.is_finite()) {
        return Err(BrainError::Invalid(
            "weight_tomography_dft_input_invalid".into(),
        ));
    }
    let n = values.len();
    ...
    for frequency in 1..=n / 2 {
        ...
        if !re.is_finite() || !im.is_finite() {
            return Err(BrainError::Numerical(
                "weight_tomography_dft_nonfinite".into(),
            ));
        }
...
fn normalized_power(values: &[f64]) -> BrainResult<(Vec<f64>, f64)> {
    ...
    if power.iter().any(|value| !value.is_finite() || *value < 0.0) {
        return Err(BrainError::Numerical(
            "weight_tomography_power_nonfinite".into(),
        ));
    }
    Ok((power, slope))
}
...
fn positive_autocorrelation(values: &[f64], lag: usize) -> BrainResult<Option<f64>> {
    ...
    if !correlation.is_finite() {
        return Err(BrainError::Numerical(
            "weight_tomography_autocorrelation_nonfinite".into(),
        ));
    }
...
fn scaled_cosine(left: &[f64], right: &[f64]) -> BrainResult<Option<f64>> {
    ...
    {
        return Err(BrainError::Invalid(
            "weight_tomography_direction_shape_invalid".into(),
        ));
    }
    ...
    if !value.is_finite() {
        return Err(BrainError::Numerical(
            "weight_tomography_direction_nonfinite".into(),
        ));
    }
...
fn subaperture_metrics(channels: &[Vec<f64>]) -> BrainResult<(f64, f64, f64)> {
    ...
    for channel in channels {
        if channel.len() != length {
            return Err(BrainError::Invalid(
                "weight_tomography_channel_length_mismatch".into(),
            ));
        }
    ...
    for value in [
        ...
        if !value.is_finite() {
            return Err(BrainError::Numerical(
                "weight_tomography_subaperture_nonfinite".into(),
            ));
        }
    }
...
pub fn analyze_weight_dynamics(
    ...
    {
        return Err(BrainError::Invalid(
            "weight_tomography_input_invalid".into(),
        ));
    }
    ...
    {
        return Err(BrainError::Invalid(
            "weight_tomography_observation_contract_invalid".into(),
        ));
    }
    ...
    {
        return Err(BrainError::Numerical(
            "weight_tomography_report_nonfinite".into(),
        ));
    }
    Ok(report)
}
...
pub fn tomography_gate(observation: &WeightTomographyObservation) -> WeightTomographyGateDecision {
    ...
    let (allow, reason) = if pathological_oscillation {
        (
            false,
            WeightTomographyGateReason::BlocksPathologicalOscillation,
        )
    } else if coherent_chatter {
        (
            false,
            WeightTomographyGateReason::BlocksHighFrequencyChatter,
        )
        ...
        (
            true,
            WeightTomographyGateReason::OppositionBelowBlockThreshold,
        )
    };
...
mod tests {
    use super::*;
    use crate::digest::{ProvenanceDigest, Sha256Digest};
    ...
    fn slow_coherent_dynamics_are_deeper_than_alternating_chatter() {
        let coherent = (0..64)
            .map(|generation| {
                ...
                observation(
                    generation,
                    vec![value, 0.8 * value, 0.6 * value, 0.4 * value],
                    0.95,
                )
            })
            .collect::<Vec<_>>();
    ...
    fn poor_trajectory_closure_reduces_confidence_without_fabricating_a_veto() {
        ...
        assert_eq!(
            gate.reason,
            WeightTomographyGateReason::LowConfidenceAdvisory
        );
    }
    ...
    fn irregular_generation_spacing_is_diagnostic_but_cannot_veto() {
        ...
        assert_eq!(
            gate.reason,
            WeightTomographyGateReason::IrregularGenerationSpacingAdvisory
        );
    }
