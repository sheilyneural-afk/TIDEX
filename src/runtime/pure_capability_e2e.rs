...
use crate::foundation::digest::{
    CapabilityIrDigest, CaptureReceiptDigest, Sha256Digest, SystemEnvelopeDigest,
};
use crate::foundation::error::{BrainError, BrainResult};
use crate::foundation::identity::{CapabilityId, CapabilityNodeId, PortId, PrimitiveId};
...
fn authenticate_commitment(
    ...
    verify_state(root, &suite_registration_path(root, &value.suite_sha256), &registration)?;
    Ok(value)
}
...
fn authenticate_claim(
    ...
    verify_state(root, &claim_path(root, &commitment.manifest_sha256), &expected)
}
...
impl PureLinearExecutionOutput {
    fn to_canonical_bytes(&self) -> BrainResult<Vec<u8>> {
        ...
        canonical_json_bounded(self, MAX_RECORD_BYTES, "isolated_linear_execution_output_too_large")
    }
}
...
impl IsolatedTerminalEvaluationAuthority {
    ...
    pub fn authenticate(
        ...
        if receipt.reveal_transition != reveal_transition {
            return Err(integrity("isolated_terminal_evaluation_reveal_state_mismatch"));
        }
        let package = PureLinearExecutionPackage::from_authenticated_chain(&candidate, &reveal)?;
        let expected_output = package.execute()?;
        let expected_stdout = expected_output.to_canonical_bytes()?;
        ...
        {
            return Err(integrity("isolated_terminal_evaluation_recomputation_mismatch"));
        }
        let terminal = TerminalTransition {
            schema: TerminalChallengeSchema::Current,
            state: TerminalChallengeState::Terminal,
...
fn authenticate_reveal_transition(
    ...
    verify_state(root, &reveal_transition_path(root, &commitment.manifest_sha256), &expected)
}
...
fn claim_path(root: &Path, commitment: &Sha256Digest) -> PathBuf {
    canonical_path(root, "state/terminal_challenges/claims/by-commitment", commitment)
}

fn reveal_transition_path(root: &Path, commitment: &Sha256Digest) -> PathBuf {
    canonical_path(root, "state/terminal_challenges/revealed/by-commitment", commitment)
}

fn terminal_transition_path(root: &Path, commitment: &Sha256Digest) -> PathBuf {
    canonical_path(root, "state/terminal_challenges/terminal/by-commitment", commitment)
}
...
mod tests {
    ...
    fn fixture(label: &str) -> (PathBuf, PathBuf, CaptureReceipt, PrivateFileReference) {
        ...
        .unwrap();
        fs::write(donor.join("capability.linear.json"), serde_json::to_vec(&descriptor).unwrap())
            .unwrap();
        fs::write(donor.join("alternative.linear.json"), serde_json::to_vec(&alternative).unwrap())
            .unwrap();
    ...
    fn retained_descriptor_reaches_terminal_weights_after_donor_removal() {
        ...
        let discovery = PureCapabilityDiscoveryAuthority::open(&root)
            .unwrap()
            .discover_linear_map(capture_reference, &source, Path::new("capability.linear.json"))
            .unwrap();
    ...
    fn exact_candidate_claim_is_atomic_under_competition() {
        ...
        let second = authority
            .discover_linear_map(capture_reference, &source, Path::new("alternative.linear.json"))
            .unwrap();
    ...
    fn isolated_runner_wire_is_canonical_and_contains_no_oracle_or_authority_material() {
        ...
        assert_eq!(PureLinearExecutionPackage::from_canonical_bytes(&wire).unwrap(), package);
    ...
    fn sealed_candidate_and_execution_package_survive_unavailable_source_cas() {
        ...
        assert_eq!(reopened.execute().unwrap().outputs, vec![vec![7.0, 5.0], vec![-7.0, 8.0]]);
        assert!(!candidates.authorizes_promotion());
