...
impl AuthenticatedBytes {
    ...
    fn verify(&self, mismatch: &'static str) -> IsolationResult<()> {
        if Sha256Digest::digest_bytes(&self.bytes) != self.digest {
            return Err(IsolationError::new(IsolationErrorKind::AuthenticationFailed, mismatch));
        }
        Ok(())
    }
}
...
pub fn run_isolated(
    ...
    let input =
        create_sealed_payload("input", &request.input, 0o400, "staged_input_digest_mismatch")?;
...
fn staging_error(error: impl std::fmt::Display) -> IsolationError {
    IsolationError::new(IsolationErrorKind::StagingFailed, format!("isolation_staging_io:{error}"))
}
...
fn join_reader(
    ...
    handle
        .join()
        .map_err(|_| {
            IsolationError::new(IsolationErrorKind::MonitorFailed, "sandbox_output_reader_panicked")
        })?
        .map_err(|error| {
            IsolationError::new(
                IsolationErrorKind::MonitorFailed,
...
mod tests {
    ...
    fn isolation_limit_and_request_boundary_matrix_fails_closed() {
        ...
        for invalid in invalid_limits {
            assert_eq!(invalid.validate().unwrap_err().kind, IsolationErrorKind::InvalidContract);
        }

        let mut valid = request();
        validate_request(&valid).unwrap();
        valid.program = AuthenticatedBytes::from_trusted_bytes(Vec::new());
        assert_eq!(validate_request(&valid).unwrap_err().detail, "execution_program_size_invalid");
    ...
    fn backend_helpers_and_descriptor_manifest_validate_exact_file_properties() {
        ...
        let path = std::env::temp_dir()
            .join(format!("tidex-backend-probe-{}-{unique}", std::process::id()));
    ...
    fn staged_digest_and_reader_join_errors_are_typed() {
        let unique = std::time::SystemTime::now()
            ...
            .as_nanos();
        let path = std::env::temp_dir()
            .join(format!("tidex-digest-probe-{}-{unique}", std::process::id()));
        fs::write(&path, b"abc").unwrap();
        let mut file = File::open(&path).unwrap();
        assert_eq!(digest_file_bounded(&mut file, 3).unwrap(), Sha256Digest::digest_bytes(b"abc"));
        let mut file = File::open(&path).unwrap();
        assert_eq!(
    ...
    fn monitor_classifies_success_nonzero_timeout_and_output_limit() {
        ...
        let mut child = ChildGuard::new(child);
        let report = monitor_child(&mut child, &base_request, request_digest.clone()).unwrap();
        assert_eq!(report.termination, ExecutionTermination::NonzeroBeforePayloadConfirmation);
        assert!(!report.succeeded());
        ...
        assert_eq!(report.termination, ExecutionTermination::OutputLimitExceeded);
    ...
    fn invalid_and_unbounded_contracts_are_rejected() {
        ...
        assert_eq!(validate_request(&nul).unwrap_err().kind, IsolationErrorKind::InvalidContract);
    ...
    fn request_digest_commits_arguments_limits_and_backend_contract() {
        ...
        assert_ne!(canonical_request_digest(&relabeled_argument).unwrap(), original_digest);

        let mut relabeled_limit = original.clone();
        relabeled_limit.limits.cpu_seconds += 1;
        assert_ne!(canonical_request_digest(&relabeled_limit).unwrap(), original_digest);

        assert_eq!(canonical_request_digest(&original).unwrap(), original_digest);
    }
