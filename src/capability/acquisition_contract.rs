...
use crate::foundation::digest::{AcquisitionRequestDigest, Sha256Digest, SystemEnvelopeDigest};
...
///
/// This is intentionally distinct from `DeclaredRelativePath`: the logical
...
impl AcquisitionRequest {
    ...
    pub fn verify(&self) -> BrainResult<()> {
        ...
        if canonical.iter().cloned().collect::<Vec<_>>() != self.exclusions {
            return Err(BrainError::Integrity("acquisition_exclusions_not_canonical".into()));
        }
        ...
        if self.manifest_sha256.is_draft() || self.calculate_digest()? != self.manifest_sha256 {
            return Err(BrainError::Integrity("acquisition_request_digest_mismatch".into()));
        }
        Ok(())
    }
...
impl SystemEnvelope {
    ...
    pub(crate) fn verify_against_session(
        ...
        if request.manifest_sha256 != self.acquisition_request_sha256 {
            return Err(BrainError::Integrity("system_envelope_request_binding_mismatch".into()));
        }
        ...
        if expected_projection_roots != self.projection_roots {
            return Err(BrainError::Integrity("system_envelope_projection_roots_mismatch".into()));
        }
        let observed = Self::capture_from_session(session, request)?;
        if observed != *self {
            return Err(BrainError::Integrity("system_envelope_source_tree_mismatch".into()));
        }
        Ok(())
    }
    ...
    pub fn verify_manifest(&self) -> BrainResult<()> {
        ...
        if self.manifest_sha256.is_draft() || self.calculate_digest()? != self.manifest_sha256 {
            return Err(BrainError::Integrity("system_envelope_digest_mismatch".into()));
        }
        Ok(())
    }
    ...
    fn validate_structure(&self) -> BrainResult<()> {
        ...
        {
            return Err(BrainError::Integrity("system_envelope_structure_invalid".into()));
        }
        ...
        for entry in &self.entries {
            ...
            if entry.sha256 == zero {
                return Err(BrainError::Integrity("system_envelope_entry_digest_zero".into()));
            }
            match entry.kind {
        ...
        if total_file_bytes != self.total_file_bytes {
            return Err(BrainError::Integrity("system_envelope_total_bytes_mismatch".into()));
        }

        let mut previous_excluded: Option<&Path> = None;
        for excluded in &self.excluded_paths {
            if excluded.relative_path.is_root() {
                return Err(BrainError::Integrity("system_envelope_excluded_root_invalid".into()));
            }
                ...
                && self.completeness == SnapshotCompleteness::DeclaredScopeOnly
            {
                self.projection_roots
                    ...
                    .map(|root| {
                        ...
                        Ok((root.as_path().to_path_buf(), child.kind, child.sha256.clone()))
                    })
                    .collect::<BrainResult<Vec<_>>>()?
            } else {
...
fn ensure_disjoint_descriptors(source: &PinnedRoot, private: &PinnedRoot) -> BrainResult<()> {
    ...
    {
        return Err(BrainError::Invalid("acquisition_source_private_root_overlap".into()));
    }
    Ok(())
}
...
fn descriptor_is_ancestor(ancestor: &Stat, descendant: &OwnedFd) -> BrainResult<bool> {
    ...
    Err(BrainError::Integrity("acquisition_root_ancestry_limit_exceeded".into()))
}
...
fn ensure_same_directory_binding(before: &Stat, after: &Stat) -> BrainResult<()> {
    ...
    {
        return Err(BrainError::Integrity("acquisition_private_root_binding_changed".into()));
    }
    Ok(())
}
    ...
    match scope {
        ...
        AcquisitionScope::DeclaredPaths { roots } => {
            ...
            if roots.len() > MAX_SCOPE_ROOTS {
                return Err(BrainError::Invalid("acquisition_scope_roots_limit_exceeded".into()));
            }
            let normalized = normalize_declared_roots(roots)?;
            if normalized != *roots {
                return Err(BrainError::Invalid("acquisition_scope_roots_not_canonical".into()));
            }
            Ok(())
        }
    }
...
fn validate_scope_exclusion_separation(
    ...
    if let AcquisitionScope::DeclaredPaths { roots } = scope {
        if let Some(excluded) = exclusions.iter().next() {
            ...
            {
                return Err(BrainError::Invalid("acquisition_scope_exclusion_overlap".into()));
            }
            return Err(BrainError::Invalid("acquisition_exclusion_outside_declared_scope".into()));
        }
    }
    Ok(())
}
...
fn capture_directory(
    ...
    if depth > MAX_RELATIVE_COMPONENTS {
        return Err(BrainError::Invalid("acquisition_directory_depth_limit_exceeded".into()));
    }
    ...
    {
        return Err(BrainError::Integrity("acquisition_directory_cycle_or_alias".into()));
    }
    ...
    if context.counters.captured_directories > MAX_CAPTURED_DIRECTORIES {
        return Err(BrainError::Invalid("acquisition_directory_limit_exceeded".into()));
    }
    ...
    for entry in &mut directory {
        ...
        directory_children =
            checked_increment(directory_children, "acquisition_directory_child_count_overflow")?;
        ...
        {
            return Err(BrainError::Invalid("acquisition_traversal_limit_exceeded".into()));
        }
    ...
    for name in names {
        ...
        match FileType::from_raw_mode(inspected.stat.st_mode) {
            ...
            FileType::Symlink => {
                return Err(BrainError::Integrity("acquisition_source_symlink_forbidden".into()));
            }
            _ => {
                return Err(BrainError::Integrity(
                    "acquisition_source_special_file_forbidden".into(),
...
fn capture_file(
    ...
    let next_file_count =
        checked_increment(context.counters.file_count, "acquisition_file_count_overflow")?;
    ...
    if actual_len != expected_len {
        return Err(BrainError::Integrity("acquisition_file_length_changed_during_capture".into()));
    }
...
pub(crate) fn copy_manifest_file_descriptor_bound(
    ...
    if opened_len != expected_len {
        return Err(BrainError::Integrity("content_vault_source_length_envelope_mismatch".into()));
    }
    let executable = opened_stat.st_mode & 0o111 != 0;
...
fn ensure_expected_object(stat: &Stat, expected: FileType) -> BrainResult<()> {
    if FileType::from_raw_mode(stat.st_mode) != expected {
        return Err(BrainError::Integrity("acquisition_source_object_type_changed".into()));
    }
    Ok(())
}
...
fn ensure_same_object(before: &Stat, after: &Stat, expected: FileType) -> BrainResult<()> {
    ...
    if !same_object_identity(before, after) {
        return Err(BrainError::Integrity("acquisition_source_object_replaced".into()));
    }
    Ok(())
}
...
fn admit_manifest_path(path: &Path, context: &mut CaptureContext<'_>) -> BrainResult<()> {
    ...
    {
        return Err(BrainError::Invalid("acquisition_manifest_limit_exceeded".into()));
    }
    Ok(())
}
...
fn admit_excluded_path(
    ...
    if context.excluded_paths.len() >= MAX_EXCLUSIONS {
        return Err(BrainError::Invalid("acquisition_excluded_path_limit_exceeded".into()));
    }
...
fn validate_requested_coverage(
    ...
    for requested in selected {
        ...
        {
            return Err(BrainError::Integrity("acquisition_declared_root_not_captured".into()));
        }
    }
    Ok(())
}
...
fn validate_explicit_exclusion_coverage(
    ...
    if observed_explicit != *requested {
        return Err(BrainError::Integrity("acquisition_explicit_exclusion_not_observed".into()));
    }
    Ok(())
}
...
fn validate_observed_relative_path(path: &Path) -> BrainResult<()> {
    if !path.as_os_str().is_empty() && !has_valid_relative_path_shape(path) {
        return Err(BrainError::Invalid("acquisition_observed_path_not_portable".into()));
    }
    Ok(())
}
...
mod tests {
    ...
    fn whole_tree_capture_is_reproducible_and_detects_mutation() {
        ...
        fs::write(temp.join("src/main.rs"), b"fn main() { println!(\"changed\"); }\n").unwrap();
    ...
    fn declared_scope_is_normalized_and_never_claims_dependency_closure() {
        ...
        .unwrap();
        assert_eq!(normalized.scope, declared(vec![PathBuf::from("config"), PathBuf::from("src")]));
        ...
        let envelope = SystemEnvelope::capture(&temp, &request).unwrap();
        assert_eq!(envelope.completeness, SnapshotCompleteness::DeclaredScopeOnly);
        assert!(!envelope
            .entries
            .iter()
    ...
    fn path_wire_is_canonical_and_rejects_malformed_hex() {
        assert_eq!(path_wire::decode("7372632f6c69622e7273").unwrap(), PathBuf::from("src/lib.rs"));
        ...
        assert_eq!(serde_json::to_vec(&observed).unwrap(), serde_json::to_vec(&encoded).unwrap());
    ...
    fn hash_protocol_separates_path_name_kind_and_order() {
        ...
        let baseline = directory_digest(
            ...
            &[
                ...
                (PathBuf::from("c"), SnapshotEntryKind::Directory, second.clone()),
            ],
        )
        .unwrap();
        let changed_framing = directory_digest(
            ...
            &[
                ...
                (PathBuf::from("bc"), SnapshotEntryKind::Directory, second.clone()),
            ],
        )
        .unwrap();
        let changed_kind = directory_digest(
            Path::new("root"),
            &[
                (PathBuf::from("ab"), SnapshotEntryKind::Directory, first.clone()),
                (PathBuf::from("c"), SnapshotEntryKind::Directory, second.clone()),
            ],
        )
        .unwrap();
    ...
    fn declared_selection_includes_only_ancestors_and_descendants() {
        ...
        assert!(is_selected(Path::new("systems/memory/short_term"), &selected));
        assert!(is_selected(Path::new("systems/memory/short_term/store.rs"), &selected));
    ...
    fn an_explicitly_selected_generated_named_root_is_not_silently_excluded() {
        ...
        fs::write(temp.join("target/capability.bin"), b"required implementation").unwrap();
        let request = AcquisitionRequest::new(
