//! Receiver facts and fail-closed planning for the capability runtime.
//!
...
use crate::capability::capability_ir::CapabilityIr;
use crate::foundation::digest::{CapabilityIrDigest, Sha256Digest};
...
impl ReceiverProfile {
    pub fn validate(&self) -> BrainResult<()> {
        ...
        for region in &self.regions {
            ...
            {
                return Err(BrainError::Invalid("receiver_profile_regions_invalid".into()));
            }
            total = total
...
pub fn create_shadow_plan(
    ...
    {
        return Err(BrainError::Invalid("materialization_plan_not_admissible".into()));
    }
