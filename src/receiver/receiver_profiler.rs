...
impl ReceiverSnapshotBinding {
    ...
    pub fn validate_for(&self, profile: &ReceiverProfile) -> BrainResult<()> {
        ...
        {
            return Err(BrainError::Integrity("receiver_snapshot_binding_invalid".into()));
        }
        Ok(())
    }
...
pub fn validate_receiver_parameter_layout(
    ...
    {
        return Err(BrainError::Integrity("receiver_parameter_layout_binding_mismatch".into()));
    }
    for (block, region) in artifact.layout.blocks.iter().zip(&profile.regions) {
        ...
        {
            return Err(BrainError::Integrity("receiver_parameter_layout_region_mismatch".into()));
        }
    }
    Ok(())
}
