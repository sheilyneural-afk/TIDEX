...
use crate::foundation::identity::{ArchitectureId, ModelId, TensorId};
use crate::receiver::architecture_families::{
    fingerprint_architecture, ArchitectureFamilyFingerprint,
};
use crate::receiver::model_adaptation::authenticate_live_receiver_model_profile;
...
use crate::receiver::weight_actuator::{inspect_model_safetensors, ModelTensorSpec};
...
fn confined(root: &Path, relative: &Path) -> BrainResult<PathBuf> {
    ...
    if !path.metadata()?.is_file() || !confined_file_target_allowed(&canonical_root, &path) {
        return Err(BrainError::Invalid("checkpoint_path_not_confined_file".into()));
    }
    Ok(path)
}
...
fn auxiliary_bytes(path: &Path, maximum: u64) -> BrainResult<Vec<u8>> {
    ...
    if bytes.is_empty() || bytes.len() as u64 > maximum {
        return Err(BrainError::Invalid("checkpoint_auxiliary_size_invalid".into()));
    }
    Ok(bytes)
}
...
pub fn inspect_safetensors_receiver(
    ...
    {
        return Err(BrainError::Invalid("safetensors_receiver_request_invalid".into()));
    }
    ...
    let config_bytes =
        auxiliary_bytes(&confined(root, &request.configuration_file)?, MAX_AUXILIARY_BYTES)?;
...
pub fn planning_profile_from_physical(
    ...
    if Sha256Digest::digest_bytes(&config) != source.config.sha256 {
        return Err(BrainError::Integrity("physical_planning_config_changed".into()));
    }
    let mut all = BTreeMap::new();
...
mod tests {
    ...
    fn inspects_exact_safetensors_geometry_and_rejects_ambiguous_dtype() {
        ...
        assert_ne!(inspected.snapshot.model_snapshot_sha256, Sha256Digest::zero());
    ...
    fn accepts_only_repository_local_huggingface_blob_symlinks() {
        ...
        let repo = std::env::temp_dir()
            .join(format!("models--fixture--hf-{}-{nonce}", std::process::id()));
        ...
        fs::write(blobs.join("config"), br#"{"model_type":"bert","architectures":["BertModel"]}"#)
            .unwrap();
        ...
        assert_eq!(
            ...
            crate::receiver::architecture_families::ModelFamily::EncoderTransformer
        );
