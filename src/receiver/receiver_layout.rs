...
use crate::analysis::block_tomography::ParameterLayoutArtifact;
use crate::foundation::digest::Sha256Digest;
use crate::foundation::error::{BrainError, BrainResult};
use crate::foundation::identity::TensorId;
use crate::receiver::receiver_profile::ReceiverProfile;
...
impl ReceiverTensorPartitioning {
    fn validate(&self, shape: &[usize]) -> BrainResult<()> {
        if let Self::Sharded { axis, partitions } = self {
            ...
            {
                return Err(BrainError::Invalid("receiver_shard_contract_invalid".into()));
            }
            ...
            for (index, partition) in partitions.iter().enumerate() {
                ...
                {
                    return Err(BrainError::Invalid("receiver_shard_partition_invalid".into()));
                }
            ...
            if usize::try_from(expected_start).ok() != Some(shape[axis]) {
                return Err(BrainError::Invalid("receiver_shard_coverage_invalid".into()));
            }
        }
        Ok(())
    }
...
impl ReceiverMaterializationLayout {
    ...
    fn validate_geometry(&self, profile: &ReceiverProfile) -> BrainResult<()> {
        ...
        {
            return Err(BrainError::Invalid("receiver_materialization_layout_invalid".into()));
        }
