...
use crate::analysis::identifiability::{resolution_map, ResolutionMap};
use crate::analysis::protected::project_to_safe_subspace;
use crate::analysis::tomography::reconstruct_skill_fields;
use crate::analysis::transport::{
...
use crate::analysis::trust_region::{apply_quadratic_trust_region, TrustRegionResult};
...
use crate::foundation::contracts::{BrainConfig, ProtectedCortex};
use crate::foundation::digest::Sha256Digest;
use crate::foundation::error::{BrainError, BrainResult};
use crate::foundation::identity::CapabilityId;
use crate::foundation::linalg::{cosine, dot, norm, stable_rms, weighted_normal_solve, Matrix};
...
impl ReceiverCompilerPolicy {
    pub fn validate(&self) -> BrainResult<()> {
        ...
        {
            return Err(BrainError::Invalid("receiver_compiler_policy_invalid".into()));
        }
        Ok(())
    }
}
...
pub struct ReceiverCalibrationSet {
    ...
    pub receiver_snapshot_binding_sha256: Option<crate::foundation::digest::Sha256Digest>,
...
pub fn benchmark_receiver_signature(
    ...
    {
        return Err(BrainError::Invalid("receiver_signature_benchmark_input_invalid".into()));
    }
...
pub fn benchmark_receiver_basis(
    ...
    {
        return Err(BrainError::Invalid("receiver_basis_duplicate_calibration_id".into()));
    }
    if input
        .calibration_deltas
        .iter()
    ...
    {
        return Err(BrainError::Invalid("receiver_basis_calibration_shape_or_values".into()));
    }
    let raw_rms = stable_rms(input.calibration_deltas.iter().flatten().copied())?;
    if raw_rms <= 0.0 {
        return Err(BrainError::Numerical("receiver_basis_zero_calibration_energy".into()));
    }
    ...
    {
        return Err(BrainError::Integrity("receiver_basis_tomography_lineage_or_shape".into()));
    }
    tomography
        .coefficients
    ...
    for (axis, axis_values) in axes.iter().enumerate().take(rank) {
        ...
        if norm(&difference)? > tolerance * norm(axis_values)?.max(1.0) {
            return Err(BrainError::Integrity("receiver_basis_source_mixture_mismatch".into()));
        }
    }
    ...
    if (reconstruction_rms - tomography.reconstruction_rms).abs() > tolerance * raw_rms {
        return Err(BrainError::Integrity("receiver_basis_reconstruction_rms_mismatch".into()));
    }
    ...
    {
        return Err(BrainError::Numerical("receiver_basis_raw_retained_energy_invalid".into()));
    }
...
fn validate_rows(
    ...
    crate::foundation::validation::validate_rows_expected(rows, expected_dim, label)
}
...
pub fn compile_receiver_capability(
    ...
    let numerical =
        compile_receiver_signature(&requested, calibration, protected_cortex, risk_metric, policy)?;
...
pub fn compile_receiver_readout_capability(
    ...
    if input.projection_components.len() > 128 || input.projection_mean.len() > 128 {
        return Err(BrainError::Invalid("receiver_readout_projection_budget".into()));
    }
    let execution =
        execute_linear_readout(input.ir, input.envelope, input.readout_weights, input.inputs)?;
...
impl FrozenTransportMap {
    ...
    fn to_map(&self) -> BrainResult<TransportMap> {
        ...
        {
            return Err(BrainError::Integrity("frozen_receiver_transport_map_invalid".into()));
        }
        Ok(TransportMap {
    ...
    fn to_map(&self) -> BrainResult<FunctionalTransplantMap> {
        ...
        if self.schema != "tidex.functional_transplant/v1"
            ...
            || [self.loo_cv_r2, self.mean_loo_cosine, self.min_loo_cosine]
                ...
                .any(|value| !value.is_finite())
        {
            return Err(BrainError::Integrity("frozen_receiver_decoder_map_invalid".into()));
        }
...
impl FrozenValidatedTransportMap {
    ...
    fn to_map(&self) -> BrainResult<ValidatedTransportMap> {
        ...
        if self.schema != "tidex.validated_transport/v1"
            ...
            || self.loo_cv_rms < 0.0
        {
            return Err(BrainError::Integrity("frozen_receiver_encoder_map_invalid".into()));
        }
...
impl FrozenReceiverCompilerMaps {
    ...
    fn to_maps(&self, calibration: &ReceiverCalibrationSet) -> BrainResult<ReceiverCompilerMaps> {
        ...
        if let Some(relational) = &self.relational {
            ...
            {
                return Err(BrainError::Integrity("frozen_receiver_relational_map_invalid".into()));
            }
        }
        ...
        {
            return Err(BrainError::Integrity("frozen_receiver_map_diagnostics_invalid".into()));
        }
...
fn fit_receiver_compiler_maps(
    ...
    let minimum_functional_anchor_separation = if calibrated_affine {
        Some(validate_functional_anchor_identity(&calibration.functional_signatures)?)
    } else {
        None
    };
    let (decoder, decoder_fit_diagnostics, encoder, encoder_fit_diagnostics) = if calibrated_affine
    {
        ...
        (decoder, Some(decoder_diagnostics), encoder, Some(encoder_diagnostics))
    } else {
...
pub fn freeze_receiver_compiler(
    ...
    {
        return Err(BrainError::Invalid("frozen_receiver_compiler_input_invalid".into()));
    }
    let work = (n as u128 + 1)
        * ((k as u128 + 1) * (f as u128 + 1).pow(3) + (f as u128 + 1) * (k as u128 + 1).pow(3));
    if work > 250_000_000 {
        return Err(BrainError::Invalid("frozen_receiver_compiler_work_limit".into()));
    }
    validate_rows(&calibration.functional_signatures, Some(f), "frozen_functional")?;
    ...
    {
        return Err(BrainError::Integrity("frozen_receiver_calibration_gates_failed".into()));
    }
    let (_, maximum_functional_leverage) = crate::analysis::transport::functional_support_envelope(
        &calibration.functional_signatures,
...
impl FrozenReceiverCompiler {
    ...
    pub fn verify(&self) -> BrainResult<VerifiedFrozenReceiverCompiler<'_>> {
        ...
        {
            return Err(BrainError::Integrity("frozen_receiver_compiler_source_mismatch".into()));
        }
        ...
        if manifest != self.manifest_sha256 {
            return Err(BrainError::Integrity("frozen_receiver_compiler_manifest_mismatch".into()));
        }
        ...
        let (_, leverage) = crate::analysis::transport::functional_support_envelope(
    ...
    pub fn validate_binding(
        ...
        {
            return Err(BrainError::Integrity("frozen_receiver_compiler_binding_mismatch".into()));
        }
        Ok(())
    }
}
...
impl VerifiedFrozenReceiverCompiler<'_> {
    ...
    pub fn compile(
        ...
        if wrong_functional_signatures.is_empty() || wrong_functional_signatures.len() > 256 {
            return Err(BrainError::Invalid("frozen_receiver_wrong_signature_cardinality".into()));
        }
        ...
        let leverage = crate::analysis::transport::functional_leverage(
...
fn relational_receiver_proposal_from_map(
    ...
    if transplant.target_coefficients.len() != calibration.receiver_solutions.len() {
        return Err(BrainError::Integrity("receiver_compiler_relational_coefficient_count".into()));
    }
        ...
        .zip(&calibration.receiver_solutions)
    {
        if anchor.len() != receiver_dim {
            return Err(BrainError::Invalid("receiver_compiler_relational_receiver_shape".into()));
        }
...
fn compile_signature_with_maps(
    ...
    if n > 256 || f > 256 || k > 256 {
        return Err(BrainError::Invalid("receiver_compiler_resource_budget_exceeded".into()));
    }
    ...
    if norm(requested)? <= 1e-15 {
        return Err(BrainError::Invalid("receiver_compiler_query_degenerate".into()));
    }
    if calibration.functional_signatures.len() != calibration.receiver_solutions.len()
        || calibration.functional_signatures.len() < 5
    {
        return Err(BrainError::Invalid("receiver_compiler_calibration_count".into()));
    }
    validate_rows(
        &calibration.functional_signatures,
        Some(requested.len()),
    ...
    let receiver_dim =
        validate_rows(&calibration.receiver_solutions, None, "receiver_compiler_receiver_anchors")?;
    if calibration.wrong_functional_signatures.is_empty() {
        return Err(BrainError::Invalid("receiver_compiler_wrong_skill_set_empty".into()));
    }
    validate_rows(
    ...
    {
        return Err(BrainError::Invalid("receiver_compiler_wrong_signature_degenerate".into()));
    }
    ...
    let (
        ...
        ReceiverProposalMethod::DecodeThenProject | ReceiverProposalMethod::CalibratedAffine => {
            (decoder.transplant(requested)?.target_vector, true, None, None, None, None)
        }
        ReceiverProposalMethod::FitProtectedCoordinates => (
...
pub fn benchmark_receiver_portability_leave_one_out(
    ...
    if input.schema != "tidex.receiver_portability_benchmark_input/v1"
        ...
        || input.cases.len() < 6
    {
        return Err(BrainError::Invalid("receiver_portability_benchmark_input_invalid".into()));
    }
    ...
    if functional_dim == 0 || receiver_dim == 0 {
        return Err(BrainError::Invalid("receiver_portability_benchmark_dimension_invalid".into()));
    }
    ...
    for case in &input.cases {
        ...
        {
            return Err(BrainError::Invalid("receiver_portability_benchmark_case_invalid".into()));
        }
    }
...
mod tests {
    ...
    fn receiver_basis_adapter_preserves_real_source_mixtures_and_row_lineage() {
        ...
        assert_eq!(result.calibration_capability_ids, input.calibration_capability_ids);
    ...
    fn receiver_readout_compilation_uses_executed_ir_values_and_projection() {
        use crate::capability::capability_ir::ParameterSlot;
        let nonce = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir()
            .join(format!("tidex-readout-compiler-{}-{nonce}", std::process::id()));
        ...
        .unwrap();
        assert_ne!(report.execution.weights_sha256, changed.execution.weights_sha256);
    ...
    fn fixture_ir() -> (PathBuf, CapabilityIr) {
        let nonce = SystemTime::now()
            ...
            .as_nanos();
        let root = std::env::temp_dir()
            .join(format!("tidex-receiver-compiler-{}-{nonce}", std::process::id()));
    ...
    fn frozen_compiler_rejects_rehashed_forged_stored_maps() {
        let frozen =
            freeze_receiver_compiler(&frozen_test_input(ReceiverProposalMethod::DecodeThenProject))
                .unwrap();
    ...
    fn protected_coordinate_fit_preserves_the_requested_response_without_weakening_gates() {
        ...
        assert_eq!(legacy.proposal_method, ReceiverProposalMethod::DecodeThenProject);
        assert_eq!(fitted.proposal_method, ReceiverProposalMethod::FitProtectedCoordinates);
    ...
    fn protected_coordinate_fit_cannot_restore_a_hard_forbidden_direction() {
        ...
        let cortex = ProtectedCortex {
            ...
            directions: vec![crate::foundation::contracts::ProtectedDirection {
                probe_id: crate::foundation::identity::ProbeId::parse("hard.x").unwrap(),
