...
use crate::artifact::{read_dvec_f32, ArtifactWriteAuthority, DeltaArtifactRef};
use crate::authority::{
...
use crate::checkpoint_adapter::{planning_profile_from_physical, PhysicalPlanningProfileRequest};
use crate::digest::Sha256Digest;
use crate::error::{BrainError, BrainResult};
use crate::identity::{CapabilityId, TensorId};
use crate::low_rank_shadow_materializer::{
    factor_dense_delta_verified, LowRankShadowPolicy, VerifiedLowRankFactors,
};
use crate::model_adaptation::{authenticate_live_receiver_model_profile, ReceiverModelProfile};
use crate::receiver_layout::ReceiverMaterializationLayout;
use crate::receiver_profile::MaterializationStrategy;
use crate::receiver_weight_binding::{
...
use crate::sparse_shadow_materializer::{sparsify, SparseCoordinate, SparseShadowPolicy};
...
fn derive(root: &Path, input: &PhysicalMaterializationRequest) -> BrainResult<Derived> {
    ...
    let (capability_id, layout, existing_delta, source_values) = match &input.source {
        CompiledMaterializationSource::MeasuredReceiver { candidate } => {
                ...
                == Some("cerebro.tidex.receiver_response_protocol/v1")
            {
                    ...
                    || protocol.tokenizer_sha256 != profile.tokenizer.sha256
                {
                    return Err(integrity(
                        "physical_materialization_tokenizer_or_config_mismatch",
                    ));
                }
            }
            ...
            (
                candidate.target_capability_id,
                basis.layout,
                Some(delta),
                values,
            )
    ...
    {
        return Err(invalid(
            "physical_backend_changes_compiled_delta_requires_fresh_validation",
        ));
    }
    Ok(Derived {
        profile,
...
pub fn authenticate_compiled_checkpoint(
    ...
    {
        return Err(integrity(
            "physical_materialization_receipt_contract_invalid",
        ));
    }
