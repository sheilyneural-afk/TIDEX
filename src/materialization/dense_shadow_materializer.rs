//! Inert, replay-bound dense-delta materialization for staging receivers.
//!
...
impl ShadowDenseDeltaCandidate {
    ...
    pub fn validate(
        ...
        if self != &expected {
            return Err(BrainError::Integrity("shadow_dense_delta_candidate_invalid".into()));
        }
        Ok(())
    }
}
...
fn build_candidate(
    ...
    if shadow.materialization_plan.strategy != MaterializationStrategy::DenseDelta {
        return Err(BrainError::Invalid("shadow_dense_delta_strategy_required".into()));
    }
    ...
    {
        return Err(BrainError::Integrity("shadow_dense_target_delta_invalid".into()));
    }
    let affected = shadow
...
pub fn persist_dense_delta_shadow(
    ...
    {
        return Err(BrainError::Invalid("shadow_dense_candidate_too_large".into()));
    }
...
pub fn load_dense_delta_shadow(
    ...
    let bytes = reference.read_verified_bounded(roots.staging_root(), MAX_DENSE_SHADOW_BYTES)?;
    ...
    if reference.path != expected_path {
        return Err(BrainError::Integrity("shadow_dense_candidate_path_invalid".into()));
    }
    candidate.validate(request, receipt, layout)?;
    Ok(candidate)
}
