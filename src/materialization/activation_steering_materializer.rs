...
impl ActivationSteeringLayout {
    ...
    fn validate_geometry(
        ...
        {
            return Err(BrainError::Invalid("activation_steering_layout_invalid".into()));
        }
        ...
        for hook in &self.hooks {
            if hook.hook_id.trim().is_empty()
                ...
                || !hook.gain.is_finite()
            {
                return Err(BrainError::Invalid("activation_hook_contract_invalid".into()));
            }
            ...
            {
                return Err(BrainError::Invalid("activation_projection_shape_invalid".into()));
            }
            projection_elements =
                projection_elements
    ...
    pub fn validate_for(&self, receiver_layout: &ReceiverMaterializationLayout) -> BrainResult<()> {
        ...
        if self.manifest_sha256 != self.calculate_digest()? {
            return Err(BrainError::Integrity("activation_steering_layout_digest_invalid".into()));
        }
        Ok(())
    }
...
impl ActivationSteeringPolicy {
    fn validate(&self) -> BrainResult<()> {
        ...
        {
            return Err(BrainError::Invalid("activation_steering_policy_invalid".into()));
        }
        Ok(())
    }
...
impl ShadowActivationSteeringCandidate {
    ...
    pub fn validate(
        ...
        if self != &build_candidate(request, receipt, receiver_layout, steering_layout, policy)? {
            return Err(BrainError::Integrity("activation_steering_candidate_invalid".into()));
        }
        Ok(())
    }
}
...
fn build_candidate(
    ...
    {
        return Err(BrainError::Invalid("activation_steering_strategy_required".into()));
    }
    let target = &receipt
        .shadow_plan
        .compilation_receipt
    ...
    for hook in &steering_layout.hooks {
        ...
        {
            return Err(BrainError::Numerical("activation_steering_vector_not_admissible".into()));
        }
...
pub fn persist_activation_steering_shadow(
    ...
    if u64::try_from(bytes.len())
        ...
        .is_none()
    {
        return Err(BrainError::Invalid("activation_steering_candidate_too_large".into()));
    }
    let path = roots
        .artifact_root()
...
pub fn load_activation_steering_shadow(
    ...
    let bytes = reference.read_verified_bounded(roots.staging_root(), MAX_STEERING_BYTES)?;
    ...
    if reference.path != expected {
        return Err(BrainError::Integrity("activation_steering_candidate_path_invalid".into()));
    }
    candidate.validate(request, receipt, receiver_layout, steering_layout, policy)?;
    Ok(candidate)
}
