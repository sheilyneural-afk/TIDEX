...
//! immutable staging artifact for a later, separately reviewed backend.
...
fn materialize_receiver_coordinates_shadow(
    ...
    {
        return Err(BrainError::Invalid("shadow_receiver_coordinate_candidate_invalid".into()));
    }
...
pub fn load_shadow_candidate(
    ...
    let bytes =
        reference.read_verified_bounded(roots.staging_root(), MAX_SHADOW_CANDIDATE_BYTES)?;
    ...
    if reference.path != expected {
        return Err(BrainError::Integrity("shadow_candidate_path_invalid".into()));
    }
    candidate.validate(profile, plan)?;
    Ok(candidate)
}
...
mod tests {
    use super::*;
    use crate::foundation::digest::CapabilityIrDigest;
    use crate::foundation::identity::{ArchitectureId, CapabilityId, ModelId, TensorId};
    use crate::receiver::receiver_profile::{
    ...
    fn persisted_candidate_is_confined_authenticated_and_replayable() {
        ...
        let base = std::env::temp_dir()
            .join(format!("cerebro-shadow-candidate-{}-{nonce}", std::process::id()));
        let staging = base.join("staging");
        let state = staging.join("state");
        let artifacts = staging.join("artifacts");
        let production = base.join("production");
        for path in [&staging, &state, &artifacts, &production] {
            fs::create_dir_all(path).unwrap();
            crate::foundation::security::secure_dir(path).unwrap();
        }
        ...
        assert_eq!(load_shadow_candidate(&roots, &profile, &plan, &reference).unwrap(), candidate);
        assert_eq!(
