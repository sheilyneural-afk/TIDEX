...
pub fn create_private_staging_file(
    ...
    loop {
        ...
        let temp_name = format!(".{file_name}.{}.{}.immutable.tmp", std::process::id(), sequence);
...
/// Content identity of an immutable file under a verified TIDE-X private root.
///
...
impl PrivateFileReference {
        ...
        F: FnOnce(),
    {
        let opened = open_private_reference(root, &self.path)?;
        after_open();
        verify_opened_private_file(opened.file, &opened.stat, &self.sha256, Some(max_bytes), true)
    }
}
...
fn validate_private_root_stat(stat: &Stat) -> BrainResult<()> {
    if FileType::from_raw_mode(stat.st_mode) != FileType::Directory {
        return Err(BrainError::Integrity("private_root_descriptor_invalid".into()));
    }
    Ok(())
}
...
fn validate_private_file_stat(root: &Stat, file: &Stat) -> BrainResult<()> {
    ...
    if file.st_uid != root.st_uid || file.st_mode & 0o7002 != 0 || group_write_exposed {
        return Err(BrainError::Integrity("private_file_permissions_or_owner_invalid".into()));
    }
    Ok(())
}
...
fn verify_opened_private_file(
    ...
    if max_bytes.is_some_and(|limit| opened_len > limit) {
        return Err(BrainError::Invalid("private_file_read_limit_exceeded".into()));
    }
    ...
    if actual_len != opened_len {
        return Err(BrainError::Integrity("private_file_mutated_during_read".into()));
    }
    let actual = Sha256Digest::parse(format!("{:x}", hasher.finalize()))?;
    if actual != *expected {
        return Err(BrainError::Integrity("private_file_reference_digest_mismatch".into()));
    }
    Ok(bytes)
}
...
pub fn root_relative_path(root: &Path, raw: &Path) -> BrainResult<PathBuf> {
    if !raw.is_absolute() {
        return Err(BrainError::Invalid("private_file_path_must_be_absolute".into()));
    }
    let relative = raw
        .strip_prefix(root)
...
fn collect_private_tree_entries_descriptor_bound(
    ...
    if depth > PRIVATE_TREE_MAX_DEPTH {
        return Err(BrainError::Invalid("private_directory_tree_depth_limit_exceeded".into()));
    }
    ensure_same_private_tree_object(
    ...
    if !visited_directories.insert((opened_directory_stat.st_dev, opened_directory_stat.st_ino)) {
        return Err(BrainError::Integrity("private_directory_tree_cycle_or_alias".into()));
    }
...
pub fn move_private_directory_transactional(
    ...
    if source == destination {
        return Err(BrainError::Invalid("private_directory_move_source_equals_destination".into()));
    }
    ...
    for component in parent.components() {
        ...
        let child_fd = match openat(
            ...
            Err(e) if e == rustix::io::Errno::NOTDIR || e == rustix::io::Errno::LOOP => {
                return Err(BrainError::Integrity("private_file_parent_not_directory".into()));
            }
            Err(e) => return Err(rustix_error(e)),
        };
...
pub fn ensure_private_directory(root: &Path, directory: &Path) -> BrainResult<PathBuf> {
    ...
    for component in relative.components() {
        let Component::Normal(name) = component else {
            return Err(BrainError::Invalid("private_directory_path_not_confined".into()));
        };
        cursor.push(name);
        let child_fd = match openat(
            &current_fd,
            ...
            Err(e) if e == rustix::io::Errno::NOTDIR || e == rustix::io::Errno::LOOP => {
                return Err(BrainError::Integrity("private_directory_target_invalid".into()));
            }
            Err(e) => return Err(rustix_error(e)),
        };
        let stat = fstat(&child_fd).map_err(rustix_error)?;
...
pub fn create_private_immutable(
    ...
    if existing_regular_file_if_present(root, path)?.is_some() {
        return Err(BrainError::Integrity("private_file_immutable_target_already_exists".into()));
    }
    let expected = Sha256Digest::digest_bytes(bytes);
    let temporary = stage_private_bytes(root, path, bytes, &expected)?;
    if install_private_immutable_file(root, &temporary, path, &expected)? {
        ...
        Err(BrainError::Integrity("private_file_immutable_target_already_exists".into()))
    }
}
...
pub fn install_private_immutable_file(
    ...
    if temporary == destination {
        return Err(BrainError::Invalid("private_file_staging_equals_destination".into()));
    }
    ensure_private_parent(root, destination)?;
    let root_fd = open_private_root_fd(root)?;
    ...
    let prepared = (|| -> BrainResult<()> {
        ...
        if sha256_fd(&mut staging_fd)? != *expected {
            return Err(BrainError::Integrity("private_file_staged_content_mismatch".into()));
        }
        Ok(())
    })();
...
pub fn move_private_file_transactional(
    ...
    if source == destination {
        return Err(BrainError::Invalid("private_file_move_source_equals_destination".into()));
    }
    ...
    if sha256_fd(&mut source_fd)? != *expected {
        return Err(BrainError::Integrity("private_file_move_source_digest_mismatch".into()));
    }
...
fn verify_immutable_content(root: &Path, path: &Path, expected: &Sha256Digest) -> BrainResult<()> {
    ...
    if sha256_fd(&mut file_fd)? != *expected {
        return Err(BrainError::Integrity("private_file_immutable_content_mismatch".into()));
    }
    ...
    F: FnOnce() -> BrainResult<()>,
{
    ...
    if expected_sha256.is_some_and(|expected| expected != &digest) {
        return Err(BrainError::Integrity("private_file_atomic_expected_digest_mismatch".into()));
    }
    ...
    let sequence = NEXT_IMMUTABLE_TEMPORARY.fetch_add(1, Ordering::Relaxed);
    let temp_name = format!(".{file_name}.{}.{}.atomic.tmp", std::process::id(), sequence);
    ...
    if let Err(e) = renameat(&parent_fd, temp_name.as_str(), &parent_fd, file_name) {
        let _ = unlinkat(&parent_fd, temp_name.as_str(), AtFlags::empty());
        let _ = parent_fd.sync_all();
        return Err(BrainError::Io(std::io::Error::from_raw_os_error(e.raw_os_error())));
    }
    after_rename()?;
    ...
    if sha256_fd(&mut dest_fd)? != digest {
        return Err(BrainError::Integrity("private_file_atomic_postwrite_mismatch".into()));
    }
    dest_fd.sync_all()?;
...
mod tests {
    ...
    fn root(label: &str) -> PathBuf {
        ...
        let root = std::env::temp_dir()
            .join(format!("cerebro-authority-{label}-{}-{nonce}", std::process::id()));
    ...
    fn staging_installer_rejects_symlink_collision_without_touching_target() {
        ...
        assert!(install_private_immutable_file(&root, &temporary, &destination, &expected).is_err());
        assert!(!temporary.exists());
    ...
    fn directory_move_preserves_complete_identity_and_retry_is_idempotent() {
        ...
        assert!(!source.exists());
        assert_eq!(inspect_private_directory(&root, &destination).unwrap(), expected);
        move_private_directory_transactional(&root, &source, &destination, &expected).unwrap();
        assert_eq!(fs::read(destination.join("nested/evidence.bin")).unwrap(), b"evidence");
        fs::remove_dir_all(root).unwrap();
    }
    ...
    fn directory_move_rejects_collision_and_tree_tampering_without_mutation() {
        ...
        assert_eq!(fs::read(destination.join("intent.json")).unwrap(), b"destination");
