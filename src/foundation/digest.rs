...
/// Canonical SHA-256 identity used across TIDE-X authority artifacts.
///
...
/// Hash an exact file byte stream using TIDE-X's canonical SHA-256 identity.
...
semantic_digest!(
    ...
    ///
    /// ```compile_fail
    /// use tidex::foundation::digest::{CheckpointDigest, EvaluationReceiptDigest};
...
semantic_digest!(
    ...
    ///
    /// ```compile_fail
    /// use tidex::foundation::digest::{AdaptiveLearningPolicyDigest, LearningTargetDigest};
...
semantic_digest!(
    ...
    ///
    /// ```compile_fail
    /// use tidex::foundation::digest::{ObservationRecordDigest, ProvenanceDigest};
...
semantic_digest!(
    ...
    ///
    /// ```compile_fail
    /// use tidex::foundation::digest::{RepresentationProtocolDigest, RepresentationRequestDigest};
...
semantic_digest!(
    ...
    ///
    /// ```compile_fail
    /// use tidex::foundation::digest::{CorpusDigest, ConfigDigest};
...
sealed_semantic_digest!(
    ...
    ///
    /// ```compile_fail
    /// use tidex::foundation::digest::{AcquisitionRequestDigest, Sha256Digest};
...
mod tests {
    ...
    fn digest_requires_canonical_lowercase_without_changing_wire_shape() {
        ...
        assert_eq!(serde_json::to_string(&digest).unwrap(), format!("\"{}\"", lower));
    }
    ...
    fn sha256_digest_traits_file_hash_and_conversions_are_canonical() {
        ...
        assert_eq!(&*raw, text);
        assert_eq!(<Sha256Digest as std::borrow::Borrow<str>>::borrow(&raw), text);
        ...
        assert_eq!(Sha256Digest::try_from(raw.as_str().to_string()).unwrap(), raw);
        ...
        let path = std::env::temp_dir()
            .join(format!("cerebro-digest-file-{}-{unique}.bin", std::process::id()));
        std::fs::write(&path, b"file-digest").unwrap();
        assert_eq!(sha256_file(&path).unwrap(), Sha256Digest::digest_bytes(b"file-digest"));
    ...
    fn semantic_digest_domains_are_distinct_but_wire_compatible() {
        ...
        assert_ne!(TypeId::of::<CheckpointDigest>(), TypeId::of::<EvaluationReceiptDigest>());
        assert_ne!(TypeId::of::<EvaluationReceiptDigest>(), TypeId::of::<ProbeProtocolDigest>());
        assert_ne!(
            TypeId::of::<LearningTargetDigest>(),
            TypeId::of::<AdaptiveLearningPolicyDigest>()
        );
        assert_ne!(TypeId::of::<ObservationRecordDigest>(), TypeId::of::<ProvenanceDigest>());
        assert_ne!(TypeId::of::<CorpusDigest>(), TypeId::of::<ConfigDigest>());
        assert_ne!(TypeId::of::<ReportDigest>(), TypeId::of::<MemoryDigest>());
        assert_ne!(TypeId::of::<CausalCreditDigest>(), TypeId::of::<ProtectedMapDigest>());
        assert_ne!(
        ...
        assert!(serde_json::from_str::<RepresentationProtocolDigest>(&format!(
            "\"{}\"",
            "AB".repeat(32)
        ))
        .is_err());
        ...
        assert_eq!(serde_json::from_str::<LearningTargetDigest>(&wire).unwrap(), learning);
        assert!(serde_json::from_str::<AdaptiveLearningPolicyDigest>(&format!(
            "\"{}\"",
            "CD".repeat(32)
        ))
        .is_err());
