...
fn fit_affine_with_policy(
    ...
    if !unit_scale.is_finite()
        ...
        || effective_ridge <= 0.0
    {
        return Err(BrainError::Numerical("transport_centered_design_degenerate".into()));
    }
    ...
    if bias.iter().any(|value| !value.is_finite()) {
        return Err(BrainError::Numerical("transport_centered_bias_nonfinite".into()));
    }
...
pub(crate) fn functional_leverage(
    ...
    {
        return Err(BrainError::Invalid("receiver_weight_functional_support_input_invalid".into()));
    }
    ...
    if !leverage.is_finite() || leverage < 0.0 {
        return Err(BrainError::Numerical("receiver_weight_functional_support_nonfinite".into()));
    }
    Ok(leverage)
}
...
pub(crate) fn functional_support_envelope(
    ...
    if calibration.len() < 5 {
        return Err(BrainError::Invalid("receiver_weight_functional_support_anchor_count".into()));
    }
...
pub fn validate_transport_with_topology(
    ...
    let target_topo =
        crate::analysis::pythagoras_topology::TopologicalSkillManifold::analyze_topology(
            target,
            distance_threshold,
        )?;
    let pred_topo =
...
pub fn learn_functional_transplant_with_policy(
    ...
    if functional_anchors.len() != target_capability_anchors.len() || functional_anchors.len() < 4 {
        return Err(BrainError::Invalid("functional_transplant_anchor_count".into()));
    }
...
impl FunctionalTransplantMap {
    pub fn transplant(
        ...
        {
            return Err(BrainError::Invalid("functional_transplant_signature_shape".into()));
        }
...
fn relational_coefficients(
    ...
    if anchors.is_empty()
        ...
        || ridge <= 0.0
    {
        return Err(BrainError::Invalid("relational_transport_coefficient_input".into()));
    }
...
fn combine_anchors(coefficients: &[f64], anchors: &[Vec<f64>]) -> BrainResult<Vec<f64>> {
    ...
    {
        return Err(BrainError::Invalid("relational_transport_combine_shape".into()));
    }
    let dim = anchors[0].len();
    if dim == 0 || anchors.iter().any(|row| row.len() != dim) {
        return Err(BrainError::Invalid("relational_transport_anchor_shape".into()));
    }
...
impl RelationalTransportMap {
    pub fn transplant(&self, source_signature: &[f64]) -> BrainResult<RelationalTransplant> {
        ...
        {
            return Err(BrainError::Invalid("relational_transplant_signature_shape".into()));
        }
