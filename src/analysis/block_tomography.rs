...
use crate::foundation::artifact::{inspect_dvec, read_dvec_range, DeltaArtifactRef};
...
use crate::foundation::security::{verify_internal_private_root, verify_private_root};
...
impl ParameterBlockLayout {
    pub fn from_shapes(specs: &[BlockShapeSpec]) -> BrainResult<Self> {
        if specs.is_empty() || specs.len() > MAX_PARAMETER_BLOCKS {
            return Err(BrainError::Invalid("block_layout_cardinality_invalid".into()));
        }
    ...
    pub fn validate(&self) -> BrainResult<()> {
        ...
        for (index, block) in self.blocks.iter().enumerate() {
            ...
            {
                return Err(BrainError::Invalid(format!("block_layout_contract_invalid:{index}")));
            }
        ...
        if expected_offset != self.total_parameter_count {
            return Err(BrainError::Invalid("block_layout_total_parameter_count_mismatch".into()));
        }
        Ok(())
    }
}
...
impl ParameterLayoutArtifact {
    ...
    pub fn validate(&self) -> BrainResult<()> {
        if self.schema != ParameterLayoutArtifactSchema::V1 {
            return Err(BrainError::Invalid("parameter_layout_artifact_schema_invalid".into()));
        }
        self.layout.validate()?;
        if self.total_parameter_count != self.layout.total_parameter_count {
            return Err(BrainError::Integrity("parameter_layout_artifact_total_mismatch".into()));
        }
...
pub fn parameter_layout_digest(
    ...
    layout.validate()?;
    Ok(ParameterLayoutDigest::from(Sha256Digest::digest_bytes(&serde_json::to_vec(
        layout,
    )?)))
}
...
impl ParameterLayoutAuthority {
    ...
    pub fn persist(&self, layout: ParameterBlockLayout) -> BrainResult<PrivateFileReference> {
        ...
        if stored != byte_sha256 {
            return Err(BrainError::Integrity("parameter_layout_artifact_write_mismatch".into()));
        }
...
fn validate_sources(
    ...
    {
        return Err(BrainError::Invalid("structured_geometry_source_count_mismatch".into()));
    }
    ...
    for (index, source) in sources.iter().enumerate() {
        ...
        {
            return Err(BrainError::Invalid(format!("structured_geometry_source_invalid:{index}")));
        }
...
fn reconstruct_block(
    ...
    for &source_index in support {
        ...
        rows.push(values);
        weights
            .push(validate_reliability(sources[source_index].reliability, "structured_geometry")?);
    }
    ...
    for (lambda, eigenvector) in eigs.iter().take(rank) {
        ...
        if axis_norm <= 1e-15 {
            return Err(BrainError::Numerical("structured_geometry_axis_degenerate".into()));
        }
...
pub fn reconstruct_structured_geometry(
    ...
    for (skill_index, field) in fields.iter().enumerate() {
        ...
        for block in &layout.blocks {
            blocks.push(reconstruct_block(root, block, sources, &support, mixture, cfg)?);
        }
...
/// Return the parameter families and transformer layers physically represented
/// by this authenticated layout. This belongs to the layout authority itself,
/// not to generic numerical validation.
pub fn layout_realization_coverage(
    layout: &ParameterBlockLayout,
) -> (std::collections::BTreeSet<String>, std::collections::BTreeSet<usize>) {
...
mod tests {
    ...
    fn layout_realization_coverage_parses_families_and_layers() {
        let layout = ParameterBlockLayout::from_shapes(&[
            BlockShapeSpec {
                name: "model.layers.0.attention.weight".into(),
                shape: vec![2, 2],
                count: 4,
            },
            BlockShapeSpec {
                name: "model.layers.1.mlp.weight".into(),
                shape: vec![2, 2],
                count: 4,
            },
        ])
    ...
    use crate::foundation::artifact::create_dvec;
    ...
    fn private_test_root(label: &str) -> PathBuf {
        ...
        let root = std::env::temp_dir()
            .join(format!("tidex-layout-{label}-{}-{nonce}", std::process::id()));
    ...
    fn block_geometry_preserves_local_rank_and_source_coefficients() {
        ...
        assert!(geometry.skills[0].blocks.iter().all(|block| {
            block
                .axes
                .iter()
